...
class BrowseDataGenerator(AbstractDataGenerator):
    def __init__(self, report, model, env):
        self.report = report
        self.model = model
        self.env = env
        self._languages = []
    ...
    def languages(self):
        ...
        ids = self.env['res.lang'].search([('translatable', '=', '1')])
        self._languages = ids.read(['code'])[0]
    ...
    def generateIds(self, record, relations, path, currentRecords):
        ...
        for relation in unrepeated:
            ...
            if root == 'Attachments':
                ...
                value = self.env['res.users'].browse([self.env.uid])
            else:
                if root == 'id':
                    value = record.id
...
class XmlBrowseDataGenerator(BrowseDataGenerator):
    ...
    def generate(self, fileName):
        ...
        for record in self.env[self.model].browse():
...
class CsvBrowseDataGenerator(BrowseDataGenerator):
    ...
    def generate(self, fileName):
        ...
        try:
            ...
            for records in self.allRecords:
                row = {}
                self.generateCsvRecord(records['root'], records, row, '', self.report.fields(), records['sequence'],
                                       records['subsequence'], records['copy'])
                writer.writerow(row)
        finally:
            f.close()
        ...
        for field in unrepeated:
            ...
            if root == 'Attachments':
                ...
                if root == 'id':
                    ...
                    value = None
                    self.warning(
                        "Field '%s' (path: %s) does not exist in model '%s'." % (root, currentPath, record._name))
            ...
            if isinstance(value, orm.browse_record_list):
                ...
                fields2 = [f.partition('/')[2] for f in fields if f.partition('/')[0] == root]
                if currentPath in records:
                    self.generateCsvRecord(records[currentPath], records, row, currentPath, fields2, sequence,
                                           subsequence, copy)
                else:
                    # If the field is not marked to be iterated use the first record only
