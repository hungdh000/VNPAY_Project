...
class Report:
    ...
    def execute(self):
        ...
        ids = self.env['ir.actions.report.xml'].search(
            [('report_name', '=', self.name[7:]), ('report_rml', 'ilike', '.jrxml')])
        data = ids.read(['report_rml', 'jasper_output'])[0]
        ...
        if self.report.language() == 'xpath':
            if self.data.get('data_source', 'model') == 'records':
                ...
                generator = CsvBrowseDataGenerator(self.report, self.model, self.env)
        ...
        for subreportInfo in self.report.subreports():
            ...
            if subreport.language() == 'xpath':
                ...
                if subreport.isHeader():
                    generator = CsvBrowseDataGenerator(subreport, 'res.users', [self.uid])
                elif self.data.get('data_source', 'model') == 'records':
                    generator = CsvRecordDataGenerator(subreport, self.data['records'])
                else:
                    generator = CsvBrowseDataGenerator(subreport, self.model, self.env)
...
if release.major_version == '5.0':
    ...
    def register_jasper_report(name, model):
        ...
        if name in odoo.report.interface.report_int._reports:
            if isinstance(odoo.report.interface.report_int._reports[name], report_jasper):
...
class ir_actions_report_xml(models.Model):
    ...
    def _lookup_report(self, name):
        ...
        self._cr.execute("SELECT * FROM ir_act_report_xml WHERE jasper_report='t' and report_name=%s limit 1", (name,))
